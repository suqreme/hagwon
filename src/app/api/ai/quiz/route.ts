    ...
    try {
      ...
      const completion = await openai.chat.completions.create({
        ...
        messages: [
          ...
          {
            role: "user",
            content: `Generate a quiz for the lesson on ${subtopic} in ${subject} for ${grade_level}. Focus on testing: ${learning_objective}. ${target_language && target_language !== 'en' ? `Please generate all questions and answers entirely in ${getLanguageName(target_language)} language.` : ''} Return the response in valid JSON format.`
          }
        ],
        temperature: 0.5,
      })
